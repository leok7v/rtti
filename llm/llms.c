...
static int is_free(const char * backend, struct model * m) {
    if (strcmp(backend, "gemini") == 0) {
        return strstr(m->id, "gemini-2.5-flash") ||
               strstr(m->id, "gemini-robotics") || strstr(m->id, "gemma");
    }
    if (strcmp(backend, "openrouter") == 0) {
        if (strstr(m->id, ":free")) { return 1; }
        return m->pricing.prompt == 0.0 && m->pricing.completion == 0.0 &&
               m->pricing.request == 0.0 && m->pricing.image == 0.0 &&
               m->pricing.web_search == 0.0 &&
               m->pricing.internal_reasoning == 0.0;
    }
    ...
    if (fp) {
        ...
        while (fgets(line, sizeof(line), fp)) {
            if (strncmp(line, "data: ", 6) == 0) {
                ...
                void * d = decode((void *)response_chunk_rtti, json);
                struct response_chunk * chunk = d;
                if (chunk && chunk->choices) {
                    ...
                    if (rc) {
                        if (!r->reasoning.data) { sb_init(&r->reasoning); }
                        sb_puts(&r->reasoning, rc);
                    ...
                    if (c) {
                        ...
                        if (!r->content.data) { sb_init(&r->content); }
                        sb_puts(&r->content, c);
...
static void run_backend(const char * backend, int bench) {
    ...
    if (!models_json) {
        ...
        void * d = decode((void *)model_list_rtti, models_json);
        struct model_list * list = d;
        free(models_json);
        if (!list) {
            ...
            if (list->data) {
                for (struct model * m = list->data; m->id && n_cand < 128;
                     m++) {
                    const char * id = m->id;
                    if (bench) {
                        if (is_free(backend, m)) { candidates[n_cand++] = id; }
                    } else if (select_model(backend, id)) {
                        candidates[n_cand++] = id;
                    }
                }
            }
            if (!n_cand) {
            ...
            if (bench) {
                printf("\n  [%s: added %d new results, total %d]\n\n", backend,
                       n_results - prev_results, n_results);
            }
            free(d);
        }
    }
...
int main(int argc, char ** argv) {
    ...
    const char * backends[] = {"simulated",  "ollama",   "gemini", "groq",
                               "openrouter", "cerebras", "github"};
    if (bench) { remove("free.csv"); }
    for (int i = 0; i < 7; i++) {
        run_backend(backends[i], bench);
    }
    /*
        if (bench) {
            int i = 1;
    ...
    */
    if (bench && n_results > 0) {
        ...
        if (f) {
            fprintf(f, "\"model_id\",\"ttft\",\"pp\",\"tg\","
                       "\"backend\",\"content\",\"reason\"\n");
