...
void generate_meta_fields(struct struct_info *s) {
    for (int j = 0; j < s->field_count; j++) {
        ...
        int is_array = 0;
        if (f->is_pointer && strcmp(kind_str, "\"'s'\"") != 0
            && strcmp(kind_str, "'s'") != 0) {
            is_array = 1;
        }
        ...
        if (is_array) {
            if (f->is_struct) {
                ...
                snprintf(size_str, sizeof(size_str),
                    "sizeof(%s)", f->type_name);
            }
        } else {
            snprintf(size_str, sizeof(size_str),
                "sizeof(((struct %s *)0)->%s)", s->name, f->field_name);
        }
        printf("  { \"%s\", offsetof(struct %s, %s),\n"
                      "    %s,\n"
                      "    %s, %d, %s },\n",
                      f->field_name, s->name, f->field_name,
                      size_str, kind_str, is_array,
                      child_meta);
    }
}
